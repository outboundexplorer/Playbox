...
function handleActionButtonClick(e) {
    ...
    switch (e.target.id) {
        ...
        case 'directionToggle4':
            ...
            break;
        case 'leftAlign':
            alignParagraph('leftAlign');
            break;
        case 'centreAlign':
            alignParagraph('centerAlign');
            break;
        case 'rightAlign':
            alignParagraph('rightAlign');
            break;
...
function alignParagraph(alignType){
...
function applyParagraphFormat(element,alignType){
    var textAlignValue = alignType.slice(0,alignType.length-5);
    console.log('textAlignvalue',textAlignValue);
    console.log('ele,ement',element);
    element.style.textAlign = textAlignValue;
}

function applyParagraphFormatsToNodes(nodes,alignType){
...
function getFlattenReadyItems(nodes){
    ...
    var _flattenReadyItem;
    for (var i = 0, iLength = nodes.length; i < iLength; i++){
        flattenReadyItem = {};
        var _computedNodeStyle = window.getComputedStyle(nodes[i].parentElement);
        console.log('computedNodeStyle',_computedNodeStyle);
        flattenReadyItem.text = nodes[i].nodeValue;
        flattenReadyItem.length = nodes[i].length;
        flattenReadyItem.color = _computedNodeStyle.color;
        flattenReadyItem.fontFamily = _computedNodeStyle.fontFamily;
        flattenReadyItem.fontWeight = _computedNodeStyle.fontWeight;
        flattenReadyItem.textDecoration = _computedNodeStyle.textDecoration;
        flattenReadyItem.fontVariant = _computedNodeStyle.fontVariant;
...
function getTextNodesFromElement(node,nodes){
    ...
    if (node.nodeType == Node.TEXT_NODE) {
        ...
        var newTextNode = document.createTextNode('\r');
